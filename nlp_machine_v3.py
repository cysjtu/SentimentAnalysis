...
from __builtin__ import str as string
...
def classify_test_file(filename, classifier, bestwords):
    data, ids = load_test_file("task2_input_cn.xml")
    features = []
    target = []
    ...
    polars = classifier.classify_many(features)

    for i in range(len(polars)):
        if polars[i] == "positive":
            target.append(1)
        else:
            target.append(-1)
    ...
    save_predict(data, ids, target, "processed_task2_output_cn.xml")
...
def main():
    filename = raw_input('请输入用于情感分析的文件名称: ')
    
    data,target = load_file()
    
    classifier, bestwords = learn_model(data,target)

    classify_test_file(filename, classifier, bestwords)
